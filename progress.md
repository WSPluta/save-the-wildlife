...
2026-06-27 refresh-rate normalized multiplayer smoothness follow-up:
  - Validated uncommitted server authority/idempotency work before committing/deploying:
    - `/usr/bin/time -lp node --check server/server.js` passed.
    - `/usr/bin/time -lp node --check scripts/controls-sign-probe.mjs` passed.
    - `/usr/bin/time -lp npm --prefix server run test:unit -- --run test/gameLogic.test.js` passed: 48 tests.
    - `/usr/bin/time -lp npm --prefix server run test:unit` passed: 82 tests.
  - Fresh public four-client multiplayer probe initially proved the important gameplay signal even though the aggregate status failed: all Chrome/WebKit desktop/mobile clients saw all human remotes, remote driver movement was about `8` world units, and remote `largeJumpCount=0`; Chrome desktop/mobile were presenting at a steady `30 FPS` in automation while WebKit held `60 FPS`.
  - Patched `scripts/browser-motion-smoothness-probe.mjs` and `scripts/multiplayer-cross-browser-sync-probe.mjs` so steady 30 Hz automation is reported as a warning when frame pacing is stable and movement deltas have no large jumps. True unstable frame pacing, missing remotes, missing motion samples, browser errors, or large jumps still fail.
  - Public normalized local motion probe passed: `.codex_tmp/qa-20260628-motion-smoothness-refresh-normalized-20260627234822/latest.md`. Chrome desktop/mobile showed steady 30 Hz warnings with `largeJumpCount=0` and max step around `0.105`; WebKit desktop/mobile passed at about `60 FPS` with max step around `0.057`. Compute: `real=102.07s`, max RSS `317079552`.
...
2026-06-30 release `69e4aa11` deployment and strict Select AI follow-up:
  - OCI DevOps build `ocid1.devopsbuildrun.oc1.uk-london-1.amaaaaaaeras5xiag6eltazeaetvy4xghrdlnptu4ihwhtb2s5ebsgmlazfa` succeeded and delivered all eight artifacts.
  - OCI DevOps deployment `ocid1.devopsdeployment.oc1.uk-london-1.amaaaaaaeras5xiaavrvchtgrb3xf5c25c2k4ovgv3ejfkphh6yatrahpkma` succeeded. OKE rolled out web `0.0.104`, server `0.0.72`, and PAF `0.0.32`; all target pods became ready with zero restarts.
  - Public score burst passed with canonical sequence `1,2,3`, final score `3`, and `scoreSource=server_room_state`; canonical lobby/admin/60-second timer probe also passed.
